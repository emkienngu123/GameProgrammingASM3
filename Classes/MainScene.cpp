...
bool MainScene::init()
{
	...
	// ¿ì¸®°¡ ¸¸µé¾î³õÀº Object Layer¿¡¼­ SpawnPoint¶ó°í ¼³Á¤ÇÑ ¼Ó¼º Á¤º¸¸¦ ÀÐ±â
	...
	// Ä³¸¯ÅÍ¸¦ SpwnPoint À§Ä¡¿¡ »ý¼ºÇÒ ¼ö ÀÖµµ·Ï ÁÂÇ¥ ÁØºñ 
	...
	for (int i = 0; i < triggerPoints.size(); i++) {
		...
		triggerCollider->setContactTestBitmask(Utils::CreateMask(core::CategoryBits::PLAYER));  // ÄÝ¸®Àü ÀÌº¥Æ®
	...
	for (int i = 0; i < bossMovePoints.size(); i++) {
		...
		bossMoveCollider->setContactTestBitmask(Utils::CreateMask(core::CategoryBits::ENEMY));  // ÄÝ¸®Àü ÀÌº¥Æ®
	...
	for (int i = 0; i < bladeBoundarys.size(); i++) {
		...
		bladeBoundaryCollider->setContactTestBitmask(Utils::CreateMask(core::CategoryBits::ENEMY_PROJECTILE));  // ÄÝ¸®Àü ÀÌº¥Æ®
...
void MainScene::createMyPhysicsWorld()
{
	...
	// ÇöÀç SceneÀÇ PhysicsWorld Á¤º¸¸¦ ¹Þ¾Æ¿À±â
	...
	// °­Ã¼ (Rigidbody) »ý¼º (¹°¸® ¿¬»êÀÌ °¡´ÉÇÑ °´Ã¼ »ý¼º)
	// PhysicsBody::createEdgeBox
	// Á¦1ÀÎ¼ö : ¹°¸® ½Ã¹Ä·¹ÀÌ¼ÇÀÌ Àû¿ëµÉ ¹üÀ§¸¦ ¼³Á¤
	// Á¦2ÀÎ¼ö : ¹°Ã¼ÀÇ ¹Ðµµ, ¹Ý¹ß°è¼ö, ¸¶Âû°è¼ö ¼³Á¤
	...
	// ¹°¸® ½Ã¹Ä·¹ÀÌ¼Ç Àû¿ëÀ» À§ÇØ ÃÖ»óÀ§ Å¬·¡½ºÀÎ Node·Î ÀÎ½ºÅÏ½º »ý¼º
